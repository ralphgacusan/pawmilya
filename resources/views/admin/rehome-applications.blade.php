    ...
    <div class="admin-container">
        ...
        <aside class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li><a href="{{ route('admin.rehome-application') }}" class="active"><i class="fas fa-home"></i>Rehoming
                        Applications</a>
                </li>
        ...
        <main class="main-content">
            ...
            <div class="table-container">
                ...
                <div class="filter-form">
                    <form action="{{ route('admin.rehome-application') }}" method="GET" class="filter-options">
                        ...
                        <div class="filter-group">
                            <label for="adoption-status-filter">Status:</label>
                            <select id="adoption-status-filter" name="status" class="form-control">
                                <option value="all"
                                ...
                                <option value="approved"
                                    {{ old('status', $filters['status'] ?? '') == 'approved' ? 'selected' : '' }}>
                                    Approved</option>
                                <option value="rejected"
                                    {{ old('status', $filters['status'] ?? '') == 'rejected' ? 'selected' : '' }}>
                                    Rejected</option>
                                <option value="completed"
                                    {{ old('status', $filters['status'] ?? '') == 'completed' ? 'selected' : '' }}>
                                    Completed</option>
                            </select>
                        </div>
                ...
                <!-- Rehome Applications Table -->
                <table>
                    <thead>
                        <tr>
                            <th>Application ID</th>
                            <th>Photo</th> <!-- Added column header -->
                    ...
                    <tbody>
                        @foreach ($rehomes as $rehome)
                            <tr>
                                <td>{{ $rehome->id }}</td>

                                <!-- Pet Photo Column -->
                                <td>
                                    @if ($rehome->rehomePet && $rehome->rehomePet->image)
                                        <img src="{{ asset('storage/' . $rehome->rehomePet->image) }}" alt="Pet Photo"
                                ...
                                <td>{{ $rehome->user->first_name }} {{ $rehome->user->last_name }}</td>
                                <td>{{ $rehome->rehomePet->name }}</td>
                                <td>{{ \Carbon\Carbon::parse($rehome->meet_up_date)->format('F d, Y') }}</td>
                                <td>{{ \Carbon\Carbon::parse($rehome->meet_up_time)->format('h:i A') }}</td>
                                <td>
                                    <span class="status {{ strtolower($rehome->status) }}">
                                        {{ ucfirst($rehome->status) }}
                                    </span>
                                </td>
                                <td>
                                    <a href="{{ route('admin.rehome-details', $rehome->id) }}">
                                        <button class="action-btn view-btn"><i class="fas fa-eye"></i></button>
                                    </a>

                                    <a href="{{ route('admin.rehome-edit', $rehome->id) }}">
                ...
                <!-- Pagination Links for Rehome Applications -->
                @if ($rehomes->hasPages())
                    <div class="pagination-container text-center mt-4">
                        <ul class="pagination justify-content-center">
                            {{-- Previous Page --}}
                            @if ($rehomes->onFirstPage())
                                <li class="page-item disabled"><span class="page-link">&laquo;</span></li>
                            @else
                                <li class="page-item">
                                    <a class="page-link" href="{{ $rehomes->previousPageUrl() }}"
                                        aria-label="Previous">
                                        &laquo;
                                    </a>
                                </li>
                            ...
                            {{-- Page Numbers --}}
                            @foreach ($rehomes->getUrlRange(1, $rehomes->lastPage()) as $page => $url)
                                <li class="page-item {{ $page == $rehomes->currentPage() ? 'active' : '' }}">
                                    <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                                </li>
                            @endforeach

                            {{-- Next Page --}}
                            @if ($rehomes->hasMorePages())
                                <li class="page-item">
                                    <a class="page-link" href="{{ $rehomes->nextPageUrl() }}" aria-label="Next">
                                        &raquo;
                                    </a>
                                </li>
                            @else
