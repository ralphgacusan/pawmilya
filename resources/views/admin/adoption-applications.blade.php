    ...
    <div class="admin-container">
        ...
        <aside class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li><a href="{{ route('admin.adopt-application') }}" class="active"><i
                            class="fas fa-file-alt"></i>Adoption
                        Applications</a>
                </li>
        ...
        <main class="main-content">
            ...
            <div class="table-container">
                ...
                <div class="filter-form">
                    <form action="{{ route('admin.adopt-application') }}" method="GET" class="filter-options">
                    ...
                    <tbody>
                        @foreach ($adoptions as $adoption)
                            <tr>
                                ...
                                <td>{{ \Carbon\Carbon::parse($adoption->meet_up_date)->format('F d, Y') }}</td>
                                <td>{{ \Carbon\Carbon::parse($adoption->meet_up_time)->format('h:i A') }}</td>
                                <td>
                                    <span class="status {{ strtolower($adoption->status) }}">
                                        {{ ucfirst($adoption->status) }}
                                    </span>
                                </td>
                                <td>
                                    <a href="{{ route('admin.adoption-details', $adoption->adoption_id) }}">
                                        <button class="action-btn view-btn"><i class="fas fa-eye"></i></button>
                                    </a>

                                    <a href="{{ route('admin.adoption-edit', $adoption->adoption_id) }}"><button
                                            class="action-btn edit-btn"><i class="fas fa-edit"></i></button></a>

                                    <form action="{{ route('client.delete-adoption', $adoption->adoption_id) }}"
            ...
            {{-- <!-- Pagination links for Adoption Applications --> --}}
            @if ($adoptions->hasPages())
                <div class="pagination-container">
                    <ul class="pagination">
                        {{-- Previous Page Link --}}
                        @if ($adoptions->onFirstPage())
                            <li class="disabled"><span>&laquo;</span></li>
                        @else
                            <li><a href="{{ $adoptions->previousPageUrl() }}">&laquo;</a></li>
                        @endif

                        {{-- Page Number Links --}}
                        @foreach ($adoptions->getUrlRange(1, $adoptions->lastPage()) as $page => $url)
                            <li class="{{ $page == $adoptions->currentPage() ? 'active' : '' }}">
                                <a href="{{ $url }}">{{ $page }}</a>
                            </li>
                        @endforeach

                        {{-- Next Page Link --}}
                        @if ($adoptions->hasMorePages())
                            <li><a href="{{ $adoptions->nextPageUrl() }}">&raquo;</a></li>
                        @else
